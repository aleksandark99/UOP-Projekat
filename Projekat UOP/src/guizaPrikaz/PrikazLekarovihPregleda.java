...
public class PrikazLekarovihPregleda extends JFrame {
	...
	public  PrikazLekarovihPregleda(DomZdravlja domzdravlja,Lekar lekar){
...
//		mainToolbar.add(btnAddPac);
		ImageIcon editIcon = new ImageIcon(getClass().getResource("/slike/zavrsen.gif"));
	    ...
	    for (Pregled pregled: domzdravlja.getPreglede()) {
	    	if (pregled.isState()==true && pregled.getLekar()==lekar) {
...
//				PrikazLekarovihPregleda.this.dispose();
//				PrikazLekarovihPregleda.this.setVisible(false);
//				
//			}
//		});
...
//								domzdravlja.obrisiPreglede(pregled);
							...
							PrikazLekarovihPregleda.this.dispose();
							PrikazLekarovihPregleda.this.setVisible(false);
							PrikazLekarovihPregleda pp=new PrikazLekarovihPregleda(domzdravlja,lekar);
							pp.setVisible(true);
		...
		btnDeletePac.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(red == -1) {
					...
					Pregled pregled = domzdravlja.nadjiPregled(Integer.parseInt(id));
					if(pregled != null &&pregled.getStatus()!=StatusPregleda.zavrsen &&pregled.getStatus()!=StatusPregleda.otkazan ) {
						int izbor = JOptionPane.showConfirmDialog(null,"Da li ste sigurni da zelite da zavrsite pregled?",pregled.getBroj() + " - Potvrda brisanja", JOptionPane.YES_NO_OPTION);
						if(izbor == JOptionPane.YES_OPTION) {
							tableModel= (DefaultTableModel) pacijentTabela.getModel();
							//DefaultTableModel model = (DefaultTableModel) pacijentTabela.getModel();
							if(pregled instanceof Pregled && pregled.getStatus()!=StatusPregleda.zavrsen) {
